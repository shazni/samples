...
public class APIMRestClient {
    ...
    public void updateSwaggerDefinition(String apiId, String swaggerContent) {
        APIMClientRegistration clientRegistration = APIMClientRegistration.getInstance();
        Map<String, String> requestHeaders = new HashMap<>();
        requestHeaders.put("Authorization", "Bearer " + clientRegistration.getUserAccessToken());
        requestHeaders.put("Content-Type", "multipart/form-data");
//        String swaggerUpdateString = "apiDefinition=\"" + swaggerContent.replace("\"", "\\\"") + "\"";

        HttpResponse swaggerUpdateResponse;
        int responseCode;

        String swaggerUpdateEp = APIMConfigurations.getInstance().getApiEnvironmentConfigList().get(0).getPublishingEndpoint() + apiId + "/swagger";

        try {
            Map<String, String> formData = new HashMap<>();
            formData.put("apiDefinition", swaggerContent);
//            swaggerUpdateResponse = HttpRequestUtil.sendRequest(new URL(swaggerUpdateEp), swaggerUpdateString, requestHeaders, Constants.HTTP_METHOD_PUT);
            swaggerUpdateResponse = HttpRequestUtil.sendMultipartRequest(new URL(swaggerUpdateEp), formData, requestHeaders, Constants.HTTP_METHOD_PUT);

            responseCode = swaggerUpdateResponse.getResponseCode();
            ...
            // Unauthorized or token expired. Let's
            if(responseCode == 401) {
                clientRegistration.generateAccessToken(APIMConfigurations.getInstance().getApiEnvironmentConfigList().get(0));
                requestHeaders.put("Authorization", "Bearer " + clientRegistration.getUserAccessToken());
                swaggerUpdateResponse = HttpRequestUtil.sendMultipartRequest(new URL(swaggerUpdateEp), formData, requestHeaders, Constants.HTTP_METHOD_PUT);
            ...
            String swaggerResponseString = swaggerUpdateResponse.getData();
            log.info("Swagger content returned = " + swaggerResponseString);
    ...
    private JsonObject buildAPICreationPayload(APIArtifact apiArtifact) {
        ...
        // In API-M 2.x below property is 'responseCaching'. In newer versions it's
        jsonApiPayLoad.addProperty("responseCachingEnabled", apiArtifact.getResponseCachingBoolean());
        ...
        if(!"".equals(apiArtifact.getBusinessInformation().getBusinessOwnerEmail())) {
...
//            jsonOutSequenceFlow.add("config", null); // This property is not needed in newer versions
...
//            jsonFaultSequenceFlow.add("config", null);      // This property is not needed in newer versions
            jsonFaultSequenceFlow.addProperty("type", "fault");

            sequenceArray.add(jsonFaultSequenceFlow);
        }

        jsonApiPayLoad.add("mediationPolicies", sequenceArray);     // Earlier this property was named 'sequences'

        jsonApiPayLoad.add("businessInformation", businessInformation);

        // Tiers do not exists in the newer version
        ...
        jsonApiPayLoad.add("policies", tiers);  // Earlier this was referred to as 'tiers'
...
//        jsonApiPayLoad.addProperty("endpointConfig", createEndpointConfig(apiArtifact));
        jsonApiPayLoad.add("endpointConfig", createEndpointConfig(apiArtifact));

        // If swagger import is enabled, swagger needs to be updated after initially creating the API
//        jsonApiPayLoad.addProperty("apiDefinition", createApiDefinitionConfig(apiArtifact));

        return jsonApiPayLoad;
    }

    // Older versions needed the output as a String
    private JsonObject createEndpointConfig(APIArtifact apiArtifact) {
...
//            jsonProductionEndpoint.add("config", null);   // This is not needed in newer versions.
...
//        return endpointJsonConfig.toString();
    ...
    public void publishTheAPI(String currentState, String targetState, String apiUUID,
                              APIMConfigurations.APIMEnvironmentConfig apiEnvironmentConfig) throws RegistryException, UnsupportedEncodingException {
        APIMClientRegistration clientRegistration = APIMClientRegistration.getInstance();
        ...
        switch(targetState) {
            case "Created":
                action = URLEncoder.encode(Constants.DEMOTE_TO_CREATED, "UTF-8");
                break;
            case "Prototyped":
                if(currentState.equals("Published")) {
                    action = URLEncoder.encode(Constants.DEMOTE_TO_PROTOTYPED, "UTF-8");
                } else {
                    action = URLEncoder.encode(Constants.DEPLOY_AS_A_PROTOTYPE, "UTF-8");
                }
                break;
            case "Published":
        ...
        try {
            ...
            // Unauthorized or token expired. Let's
            if(responseCode == 401) {
                // If token exists ideally should use refresh_token
                clientRegistration.generateAccessToken(apiEnvironmentConfig);
                requestHeaders.put("Authorization", "Bearer " + clientRegistration.getUserAccessToken());
                apiPublishResponse = HttpRequestUtil.sendRequest(new URL(changeAPIStatusEp), "", requestHeaders, Constants.HTTP_METHOD_POST);
