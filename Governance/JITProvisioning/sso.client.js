...
(function (client) {
    var log = new Log('sso-client');
    var Util = Packages.org.jaggeryjs.modules.sso.common.util.Util,
        carbon = require('carbon'),
		//publisher_config = require('/config/publisher.json'),
        log = new Log();
    ...
    client.decodeSAMLLoginResponse = function (samlObj, samlResp, sessionId) {
        ...
        if (samlObj instanceof Packages.org.opensaml.saml2.core.Response) {
			...
			log.info("Assertions " + stringify(assertions.get(0)));
            // extract the session index #####################################  MORE STUFF NEEDED HERE
            log.info("Inside sso.client.js #############################################"); 
			log.info("Number of assertions " + stringify(assertions.size()));
            if (assertions != null && assertions.size() > 0) {
				...
				log.info("Number of attribute statements " + stringify(attributeStatements.size()));			
				if(attributeStatements != null && attributeStatements.size() > 0) {
						...
						if (attributeStatement != null) {
							...
							log.info("Number of attributes " + stringify(attributes.size()));
							var attributeObj = {};
							if(attributes != null && attributes.size() > 0) {
								for(var i = 0; i < attributes.size(); i++) {
										...
										log.info("Name of the attribute " + attributeName);
										//if(attributeName = "http://wso2.org/claims/role") {
												var attributeValues = attribute.getAttributeValues();
												var attributeValuesArray = [];			
												log.info("Number of the attribute values " + attributeValues.size());
												if(attributeValues != null && attributeValues.size() > 0) {
													for(var k=0; k < attributeValues.size(); k++) {
														var xmlObject = attributeValues.get(k);
														var nodeValue = xmlObject.getDOM().getFirstChild().getNodeValue();
														log.info("Attribute Value = " + nodeValue);
														attributeValuesArray.push(nodeValue);
													}
												}

												attributeObj[attributeName] = attributeValuesArray;
												//samlSessionObj.roleAttributeValues = attributeValuesArray;
										//}
								}
								samlSessionObj.attributeObjects = attributeObj;
							}
						}
				}
				/* ##### */
